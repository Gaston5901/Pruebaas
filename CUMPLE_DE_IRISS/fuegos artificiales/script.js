...
function showIntroModal() {
	const modal = document.querySelector('.fire-modal');
	if (!modal) return;
	
	// Keep shells dormant until the user starts the show
	store.setState({ config: Object.assign({}, store.state.config, { autoLaunch: false }) });
	
	modal.classList.add('active');
	
	const btn = modal.querySelector('.fire-modal__btn');
	if (!btn) return;
	
	btn.addEventListener('click', () => {
		// Activate sound on first interaction
		toggleSound(true);
		// Hide mute, pause and settings buttons
		document.body.classList.add('sound-on');
		// Close modal
		modal.classList.remove('active');
		// Ignite the show
		store.setState({ config: Object.assign({}, store.state.config, { autoLaunch: true }) });
		launchBigOpening();
		startCountdown(120);
	});
}
...
function startCountdown(seconds) {
	const countdown = document.querySelector('.fire-countdown');
	if (!countdown) return;
	
	const timeNode = countdown.querySelector('.fire-countdown__time');
	...
	const interval = setInterval(() => {
		...
		// Kick off a bigger mix in the final stretch.
		if (!finaleStarted && remaining <= 25) {
			finaleStarted = true;
			startFinaleCombos();
		}
...
function startFinaleCombos() {
	// Turn on the built-in fast-fire mode for extra density.
	store.setState({ config: Object.assign({}, store.state.config, { finale: true }) });
	
	// Layer in more flavoring bursts on top of the auto finale.
	// On mobile, keep the extra setpieces smaller so it stays smooth.
	const extras = IS_MOBILE
		? [
			() => seqPyramid(),
			() => seqCurtainShells()
		  ]
	...
	let delay = 0;
	extras.forEach((opener, i) => {
		setTimeout(opener, delay);
		delay += 600 + i * 300;
	});
}
...
function endFinale() {
	...
	// Keep the show going, but calmer: slower speed means fewer, more spaced out combos.
	simSpeed = 0.75;
}

function showContinueButton() {
	const continueEl = document.querySelector('.fire-continue');
...
function launchBigOpening() {
	// Showrunning the opening, mixing classic shells with flair.
	// On mobile, keep the packed setpieces lighter so it stays smooth.
	const openers = IS_MOBILE
		? [
			() => seqCurtainShells(),
			() => seqPyramid(),
			() => seqTriple()
	...
	openers.forEach((opener, i) => {
		setTimeout(opener, delay);
		// spacing the set-pieces for a layered look
		delay += i%2 === 0 ? 850 : 1350;
	});
}
...
function seqCurtainShells() {
	const count = IS_DESKTOP ? 6 : 4;
	const size = Math.max(0, shellSizeSelector() - 1);
	const curtainTypes = [palmShell, willowShell, horsetailShell];

	for (let i = 0; i < count; i++) {
		const x = (i + 1) / (count + 1);
		const delay = i * 320;
		setTimeout(() => {
			const type = curtainTypes[(Math.random() * curtainTypes.length) | 0];
			const shell = new Shell(type(size));
			shell.launch(x, 0.55 + Math.random() * 0.2);
		}, delay);
	}

	return 1800 + count * 320;
}

function seqRisingBursts() {
	const count = IS_DESKTOP ? 5 : 3;
	const baseSize = shellSizeSelector();
	const size = Math.max(0, baseSize - 1);

	for (let i = 0; i < count; i++) {
		const x = 0.5 + (Math.random() * 0.5 - 0.25);
		const delay = i * 450;
		const useCrossette = Math.random() < 0.5;
		setTimeout(() => {
			const type = useCrossette ? crossetteShell : strobeShell;
			const shell = new Shell(type(size));
			shell.launch(x, 0.4 + Math.random() * 0.3);
		}, delay);
	}

	return 1400 + count * 450;
}

function seqTwirlSpins() {
	const count = IS_DESKTOP ? 8 : 5;
	const size = Math.max(0, shellSizeSelector() - 2);

	for (let i = 0; i < count; i++) {
		const angle = i / count * Math.PI * 2;
		const x = 0.5 + Math.cos(angle) * 0.32;
		const height = 0.55 + Math.sin(angle) * 0.25;
		const delay = i * 260;
		setTimeout(() => {
			const type = Math.random() < 0.5 ? ringShell : crackleShell;
			const shell = new Shell(type(size));
			shell.launch(x, height);
		}, delay);
	}

	return 1600 + count * 260;
}
...
function startSequence() {
	...
	if (rand < 0.14) {
		return seqCurtainShells();
	}
	
	if (rand < 0.18) {
		return seqRisingBursts();
	}
	
	if (rand < 0.22) {
		return seqTwirlSpins();
	}
	
	if (rand < 0.62 && !IS_HEADER) {
...
function handlePointerStart(event) {
	...
	if (!document.body.classList.contains('sound-on') && event.y < btnSize) {
		if (event.x < btnSize) {
			togglePause();
			return;
		}
...
const soundManager = {
	...
	playSound(type, scale=1) {
		...
		if (!source.buffers || source.buffers.length === 0) {
			// Audio may still be preloading in the background; skip until ready.
			return;
		}
...
if (IS_HEADER) {
	...
	// Go straight to the intro modal. Preload audio in the background so it's
	// ready by the time the user taps the button.
	init();
	setTimeout(() => {
		...
		.then(
			() => {},
			reason => {
				// Codepen preview doesn't like to load the audio, so just ignore failures.
